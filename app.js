...
function generateShareImage() {
  return new Promise((resolve) => {
    ...
    if (auxAnalysis) {
      ...
      ctx.beginPath(); ctx.roundRect(60, 185, 480, 100, 8); ctx.fill();
      ctx.strokeStyle = borderC; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.roundRect(60, 185, 480, 100, 8); ctx.stroke();
      ctx.fillStyle = text;
      ctx.font = '500 15px Source Sans 3, sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText(`${zh?'主功能':'Dom'}: ${stack[0]}`, 80, 213);
      if (auxAnalysis.balanced) {
        ctx.fillText(`${zh?'輔助傾向':'Aux'}: ${auxAnalysis.auxA} / ${auxAnalysis.auxB} (${zh?'平衡':'balanced'})`, 80, 238);
      } else {
        ctx.fillText(`${zh?'輔助傾向':'Aux'}: ${auxAnalysis.winner}(${auxAnalysis.winnerStr}) | ${auxAnalysis.loser}(${auxAnalysis.loserStr})`, 80, 238);
      }
      ctx.fillText(`${zh?'類型':'Type'}: ${state.resultType} (${t(label)})`, 80, 263);
    }
    ...
    const shortDesc = detailText.length > 60 ? detailText.substring(0, 60) + '...' : detailText;
    ctx.fillStyle = subtle;
    ctx.font = '400 14px Source Sans 3, sans-serif';
    ctx.textAlign = 'left';
    wrapText(ctx, shortDesc, 60, 310, 480, 20);
    ...
    drawRadarToCtx(ctx, state.scores, 600, 330, 220, isDark);
